...
const isMemoryLimitError = (stderr) => /killed signal terminated program|out of memory|memory limit/i.test(stderr);

const buildDockerArgs = (language, folderPath, timeoutMs = DEFAULT_EXECUTION_TIMEOUT_MS) => {
...
const executeCodeInDocker = (language, folderPath, input = "", timeoutMs = DEFAULT_EXECUTION_TIMEOUT_MS) => {
  return new Promise((resolve, reject) => {
    ...
    const getExecutionTimeMs = () => Number((process.hrtime.bigint() - startedAt) / 1000000n);
    ...
    dockerProcess.on("close", (code, signal) => {
      ...
      if (signal) {
        return finish(reject, `Execution stopped by signal: ${signal}`);
      }
